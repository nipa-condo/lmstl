...
export async function action({ request, params }: any) {
  ...
  try {
    const res = await API.sendPosttest(params.id, JSON.parse(submitData.data));
...
export const LessionsSinglePosttest = () => {
  ...
  const onFinish = (values: any) => {
    ...
    submit({ data: JSON.stringify({ posttests: result }) }, { method: "post" });
  };
  ...
  return (
    <Col>
      ...
      >
        {lession.lession &&
          lession.lession.posttests.map((data: any) => (
            <Form.Item
              key={data.id}
